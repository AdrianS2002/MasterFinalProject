...
async function getUsedRenewablePlan(contractAddress) {
    try {
        const contract = loadNodeContract(contractAddress);
        const result = await contract.getUsedRenewablePlan(); // apel Solidity

        const converted = Array.from(result).map(Number); // conversie în array JS
        return converted;
    } catch (e) {
        console.error("❌ Eroare în getUsedRenewablePlan:", e);
        throw e;
    }
}
...
async function getFlexibleLoad(contractAddress) {
    try {
        const contract = loadNodeContract(contractAddress);
        const result = await contract.getFlexibleLoad(); 
        return convertResultToArray(result); 
    } catch (e) {
        console.error("❌ Eroare în getFlexibleLoad:", e);
        throw e;
    }
}
